    ...
    def bake_output(self, proc_res, kb):
        ...
        for key,data in result.items():
            if str(key) in ["dates", "intervals"]:
                ...
                kb_data = OrderedDict()
                if item_type in kb.kb_columns:
                    columns = kb.kb_columns[item_type]
                else:
                    columns = kb.kb_columns["col"]
        ...
        return {"header":{"status":0,
                            "msg":"",
                            "processor":"",
                            "groups": dict(kb.kb_columns["prefix_desc"].items() + self.__column_enh.items()) if kb.kb_columns["prefix_desc"] is not None else None
                          },
                "result":result_kb
                }
